...
export const createTable = `CREATE TABLE IF NOT EXISTS workspaces (
  id TEXT PRIMARY KEY,
...
export async function POST(request: Request) {
  ...
  if (!body.draft && (active?.total || 0) >= 1) return json({ error: 'Finish the active audit before starting another.' }, 409);
  ...
  await db.prepare('INSERT INTO workspaces (id, user_id, created_at, repository, branch, contracts, status) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .bind(id, user.userId, now, repository.replace(/\/$/, ''), branch, JSON.stringify([...new Set(contracts)]), body.draft === true ? 'draft' : 'queued').run();
...
export async function GET(request: Request) {
  ...
  const db = getDb(); await db.prepare(createTable).run();
  const result = await db.prepare('SELECT id, repository, branch, contracts, status, created_at FROM workspaces WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').bind(user.userId).all();
  return json({ workspaces: result.results.map((row: any) => ({ ...row, contracts: JSON.parse(row.contracts) })) });
}
